...
module.exports = async(client, member) => {
    ...
    if (guild.locked) {
        await member.send(new client.embed().setDescription(`${member.guild.name} is currently under a **SERVER-WIDE LOCKDOWN!** Please try joining back a little later!`).setFooter(`${member.guild.name} | Made By dreamwastaken#0001`, member.guild.iconURL({ dynamic: true }))).catch(() => {})
        member.kick().catch(() => {})
    }
