...
module.exports.run = async(client, message, args) => {
    ...
    if (!channel) return message.channel.send(new client.embed().setDescription(`You failed to provide me where im sending this poll!`).setFooter(`${message.guild.name} | Made By dreamwastaken#0001`, message.guild.iconURL({ dynamic: true })))
    if (!args[2]) return message.channel.send(new client.embed().setDescription("You didnt specify ur question!").setFooter(`${message.guild.name} | Made By dreamwastaken#0001`, message.guild.iconURL({ dynamic: true })))
