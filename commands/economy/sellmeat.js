...
module.exports.run = async(client, message, args) => {
    ...
    if (!meat) return message.channel.send(new client.embed().setDescription(`You dont have any meat to sell! Go hunt for some with \`${message.px}meat\``).setFooter(`${message.guild.name} | Made By dreamwastaken#0001`, message.guild.iconURL({ dynamic: true })))

    const amount = 5 * meat
    message.channel.send(new client.embed().setDescription(`You sold **${meat}** for **${amount}** ${message.coin}!\nThe money has been added to your wallet`).setFooter(`${message.guild.name} | Made By dreamwastaken#0001`, message.guild.iconURL({ dynamic: true })))
