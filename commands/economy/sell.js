...
module.exports.run = async(client, message, args) => {
    ...
    if (!item) return message.channel.send(new client.embed().setDescription(`You need to enter the item id! do \`${message.px}inventory\` to view all the things you can sell!`).setFooter(`${message.guild.name} | Made By dreamwastaken#0001`, message.guild.iconURL({ dynamic: true })))
