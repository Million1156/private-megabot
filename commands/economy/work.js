...
module.exports.run = async(client, message, args) => {
    ...
    if (!data.some(s => s.name.includes('Iphone'))) return message.channel.send(new client.embed().setDescription(`You need to buy a phone to apply for a job! Buy one using \`${message.px}shop\`!`).setFooter(`${message.guild.name} | Made By dreamwastaken#0001`, message.guild.iconURL({ dynamic: true })))
