...
module.exports.run = async(client, message, args) => {
    ...
    if (!data.inventory.items.some(s => s.name.includes('Gun'))) return message.channel.send(new client.embed().setDescription(`You need to buy a gun to rob a bank! Buy one from the \`${message.px}shop\`!`).setFooter(`${message.guild.name} | Made By dreamwastaken#0001`, message.guild.iconURL({ dynamic: true })))
