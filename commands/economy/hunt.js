...
module.exports.run = async(client, message, args) => {
    ...
    if (!data.some(s => s.name.includes('Bow'))) return message.channel.send(new client.embed().setDescription(`You cant hunt without a gun! Buy one using \`${message.px}shop\`!`).setFooter(`${message.guild.name} | Made By dreamwastaken#0001`, message.guild.iconURL({ dynamic: true })))
