...
module.exports.run = async(client, message, args) => {
    ...
    if (!data2.some(s => s.name.includes('Bank'))) return message.channel.send(new client.embed().setDescription(`You need to get a bank card to withdraw money! Buy one using \`${message.px}shop\`!`).setFooter(`${message.guild.name} | Made By dreamwastaken#0001`, message.guild.iconURL({ dynamic: true })))

    if (!args[0] || (isNaN(args[0]) && args[0] !== 'all')) return message.channel.send(new client.embed().setDescription(`You need to enter how much you want to withdraw!\nExample: \`${message.px}wd 100\``).setFooter(`${message.guild.name} | Made By dreamwastaken#0001`, message.guild.iconURL({ dynamic: true })))
