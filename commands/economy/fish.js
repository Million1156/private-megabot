...
module.exports.run = async(client, message, args) => {
    ...
    if (!data.some(s => s.name.includes('Fishing'))) return message.channel.send(new client.embed().setDescription(`You cant fish without a fishing rod! Buy one using \`${message.px}shop\`!`).setFooter(`${message.guild.name} | Made By dreamwastaken#0001`, message.guild.iconURL({ dynamic: true })))
