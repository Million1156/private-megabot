...
module.exports.run = async(client, message, args) => {
    ...
    if (cooldown) return (await message.channel.send(new client.embed().setDescription(`Come back in ${duration(cooldown - Date.now(), { round: true })} to claim another daily reward`).setFooter(`${message.guild.name} | Made By dreamwastaken#0001`, message.guild.iconURL({ dynamic: true }))).delete({ timeout: 8000 }))
