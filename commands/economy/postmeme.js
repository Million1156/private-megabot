...
module.exports.run = async(client, message, args) => {
    ...
    if (!data.inventory.items.some(s => s.name.includes('Computer'))) return message.channel.send(new client.embed().setDescription(`You need to buy a computer to post a meme! Buy one from the \`${message.px}shop\`!`).setFooter(`${message.guild.name} | Made By dreamwastaken#0001`, message.guild.iconURL({ dynamic: true })))
