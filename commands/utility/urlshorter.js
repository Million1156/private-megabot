...
module.exports.run = async(client, message, args) => {
    ...
    if (!args[0]) return message.channel.send(new client.embed().setDescription(`Please provide a URL you want to shorten!`).setFooter(`${message.guild.name} | Made By dreamwastaken#0001`, message.guild.iconURL({ dynamic: true })))
    if (!regex.exec(args[0])) return message.channel.send(new client.embed().setDescription(`Please provide me a valid URL!`).setFooter(`${message.guild.name} | Made By dreamwastaken#0001`, message.guild.iconURL({ dynamic: true })))
