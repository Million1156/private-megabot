...
module.exports.run = async(client, message, args) => {
    ...
    if (!channel) return message.channel.send(new client.embed().setDescription('A report channel hasnt been setup for this server!').setFooter(`${message.guild.name} | Made By dreamwastaken#0001`, message.guild.iconURL({ dynamic: true })))
    if (!args[0]) return message.channel.send(new client.embed().setDescription(`Please provide me a report!`).setFooter(`${message.guild.name} | Made By dreamwastaken#0001`, message.guild.iconURL({ dynamic: true })));
